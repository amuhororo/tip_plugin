...
function tipLoadcsv(pm) {
  ...
  function csvjson(data) {
    ...
    const csvNew = $.grep(csvArray, function (e) {
      return e;
    }); //空行削除
...
function displayTiplist(pm) {
  ...
  $.ajax({
    ...
    //.focusable(); 保留※hover時はclass追加にしないと駄目っぽい。2ページ目から動作しないので要確認。

    //閉じるボタン
    $(".button_close").on("click mouseenter mouseleave", function (e) {
      if ($("#tip_wrap").length) return; //TIP表示中は無効
    ...
    if (tip_conf.pagefeed == "false" || tip_conf.pagefeed == "none") {
      ...
      if (tip_conf.pagefeed == "auto") {
        ...
        const count_w = Math.floor(
          parseInt($("#tip_list_container").width()) / parseInt($("#tip_list_container").find("li").outerWidth(true))
        );
      ...
      do {
        $("#tip_list_container")
          .children("li:lt(" + list + ")")
          .wrapAll("<ul class='tip_list_area'></ul>");
...
function displayTip(pm) {
  ...
  //TIPデータ取り出し
  const tip = $.grep(tipdata, function (e, i) {
    return e.key == pm.key;
  });
  //テンプレートファイル
  ...
  $.ajax({
    ...
    if ($(".tip_body .tip[data-key]").length > 0) {
      for (let i = 0; i < $(".tip_body .tip[data-key]").length; i++) {
        ...
        const _tip = $.grep(_data, function (e, i) {
          return e.key == _pm.key;
        });
        _pm.flag_val = 1;
...
function tip(pm) {
  ...
  //データにkeyが存在するかチェック
  const tip = $.grep(data, function (e, i) {
    return e.key == pm.key;
  });
...
function tipBtn(pm) {
  ...
  if (tipdata) {
    ...
    if ($("img + .tipbtn." + pm.data_name).length == 0) {
      ...
      $("img.tip_btn." + pm.data_name).after(
        '<div class="tipbtn fixlayer ' + pm.data_name + " " + pm.pos + '"><span></span></div>'
      );
    ...
    if ($(".glink_button.tip_btn." + pm.data_name + " .tipbtn").length == 0) {
      $(".glink_button.tip_btn." + pm.data_name).append(
        '<div class="tipbtn ' + pm.data_name + " " + pm.pos + '"><span></span></div>'
      );
    }
    //テキスト変更
    $(".tipbtn." + pm.data_name)
...
//--- ◆ その他 ------------------------------------------------------------------------------
//スキップ・オート・キーコンフィグ状態を保存して戻す用
function skip_save(name) {
  const tip_conf = TYRANO.kag.variable.sf.tip_conf;
  if (name == true) {
    //skip・auto
    if (tip_conf.keepOnSkip == "true") {
    ...
    //キーコンフィグ
    if (tip_conf.keyconfig == "true") {
      tip_conf.is_keyconfig = TYRANO.kag.stat.enable_keyconfig;
    ...
    if (tip_conf.is_keyconfig == false) TYRANO.kag.stat.enable_keyconfig = false;
  }
}
